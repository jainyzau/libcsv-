...
namespace CSV
{
	...
	long long CSVDocument::parse( const std::string& file_path )
	{
		...
		std::string::size_type field_beg = 0;
		std::string::size_type field_end = 0;
		row_type row;
		row_index_type row_count = 0;
		column_index_type col_count = 0;
		ParseState state = LineEnd;
		while (std::getline(csv_file, read_str))
		{
			...
			if (state == LineEnd)
			{
				...
				if (row.size() > 0)
				{
					...
					else if (row.size() > col_count)
					{
						...
						str_stream << "Syntax error: too much fields in line " << row_count << ".";
			...
			element_type elem;
			while(idx < row_str.size())
			{
				if (row_str[idx] == '"')
				{
					if (field_beg == idx && state == FieldStart)
					{
						state = FrontQuote;
					...
					else if (state == FrontQuote || state == EscapeOff)
					{
						state = EscapeOn;
					}
					else if (state == EscapeOn)
					{
						state = EscapeOff;

						//////////////////////
						if (row_str[idx - 1] != '"') // in this state, (idx - 1) is definitely valid.
						{
				...
				{
					field_end = idx;
					if (field_beg < idx && row_str[idx-1] == '"' && (state == EscapeOn || state == EscapeOff))
					{
						state = BackQuote;
					...
					if (state == BackQuote || state == FieldStart)
					{
						state = FieldEnd;
